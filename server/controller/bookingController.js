...
const getRoomsAvailable = async (
  ...
  return {
    room,
    available: room.totalRooms - booked,
  };
};
...
export const createBooking = catchAsync(async (req, res, next) => {
  ...
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) {
    return next(
      new AppError("Invalid check-in or check-out date.", 400)
    );
  }
  ...
  const { room, available } = await getRoomsAvailable(
    roomId,
    start,
    end
  );
    ...
    return next(
      new AppError(
        `Only ${Math.max(
          available,
          0
        )} room(s) available for these dates.`,
        400
      )
    );
  }
...
export const getHotelBookings = catchAsync(
  async (req, res, next) => {
